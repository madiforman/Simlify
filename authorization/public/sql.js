...
initSqlJs().then(function (SQL) {
  ...
  db.run(
    "CREATE TABLE otherUserTopTrackWeek (id, songName, artistName, genre, streams);"
  );
  db.run(
  ...
  db2.run(
    "CREATE TABLE topTrackWeek (id, songName, artistName, genre, streams);"
  );
  db2.run(
    `INSERT INTO topTrackWeek (id, songName, artistName, genre, streams)
  ...
  var stmt = db.prepare(
    "SELECT * FROM topTrackWeek NATURAL JOIN otherUserTopTrackWeek WHERE id BETWEEN $start AND $end"
  );
  ...
  var stmt2 = db2.prepare(
    "SELECT * FROM topTrackWeek WHERE id BETWEEN $start AND $end"
  );
  ...
  let acoutsticnessOne = [];
  let acoutsticnessTwo = [];

  db.all(
    "SELECT Acousticness FROM Music * Users&Music WHERE id == 1",
    [],
    (err, rows) => {
      if (err) {
      ...
      rows.forEach((row) => {
        acoutsticnessOne.push(row);
      });
    }
  );

  db.all(
    "SELECT Acousticness FROM Music * Users&Music WHERE id == 2",
    [],
    (err, rows) => {
      if (err) {
        return console.error(err.message);
